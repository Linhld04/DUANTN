window.MauSacController = function ($scope, $http) {
    ...
    $scope.onUpdate = function () {
        $http({
            method: 'PUT',
            url: `http://localhost:8080/api/admin/mau_sac/${$scope.MauSac.id}`,
            data: $scope.MauSac
        }).then(function (response) {
    ...
    $scope.onDelete = function (idMauSac) {
        if (confirm('Bạn có chắc chắn muốn xóa chất liệu này không?')) {
            $http({
                method: 'DELETE',
                url: `http://localhost:8080/api/admin/mau_sac/${idMauSac}`
            }).then(function (response) {
    ...
    $scope.onSearch = function () {
        ...
        const url = `http://localhost:8080/api/admin/mau_sac/${query}`; // URL tìm kiếm theo tên
