window.ChatLieuController = function ($scope, $http) {
    ...
    $scope.onUpdate = function () {
        $http({
            method: 'PUT',
            url: `http://localhost:8080/api/admin/chat_lieu/${$scope.ChatLieu.id}`,
            data: $scope.ChatLieu
        }).then(function (response) {
    ...
    $scope.onDelete = function (id) {
        if (confirm('Bạn có chắc chắn muốn xóa chất liệu này không?')) {
            $http({
                method: 'DELETE',
                url: `http://localhost:8080/api/admin/chat_lieu/${id}`
            }).then(function (response) {
    ...
    $scope.onSearch = function () {
        ...
        const url = `http://localhost:8080/api/admin/chat_lieu/${query}`; // URL tìm kiếm theo tên
